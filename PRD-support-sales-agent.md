...
> **v1.1 更新（2026-06-23）：** Sales Agent → Analytics（Performance）改版为「全漏斗」模型，KPI 由 4 个改为 6 个，Sales Trend 升级为 Composed Chart，Breakdown 表新增 CVR 列。详见新增章节 **§3.4 Analytics — Performance 改版（全漏斗版）**。Touchpoint 维度保持不变（Resolution Center / WFP Confirmation Email / Support Chat / Search Bar）。原 §3.2 作为历史版本保留。
...
- 图表类型：**Composed Chart（柱 + 线组合图）**，双 Y 轴。
- X 轴：日期。
- 系列与映射：

| 系列 | 图形 | Y 轴 | 颜色 |
|------|------|------|------|
| **Impressions** | 柱状 Bar | 左轴 | 紫色 `#6c47ff` |
| **Clicks** | 实线 Line | 右轴 | 青色 `#06b6d4` |
| **Orders** | 实线 Line | 右轴 | 橙色 `#f59e0b` |
| **Revenue** | 虚线 Line | 左轴 | 绿色 `#10b981` |

- 标题「Daily trend」与图例同一行（左标题，右图例）。
- Tooltip：Revenue 显示为货币格式，其余显示千分位整数。
...
3. Daily Trend 为柱 + 线 Composed Chart，双 Y 轴。
