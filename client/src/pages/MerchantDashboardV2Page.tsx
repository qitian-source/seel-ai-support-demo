 ...
 * Portal-style layout: Sidebar + header tabs (matches AI support shell)
 */
...
import {
  Star, MessageSquareText, ExternalLink,
  Globe, Pencil, Check, X, AlertCircle, CheckCircle2, PowerOff,
} from "lucide-react";
...
function PerformanceTab() {
  ...
  const [dailyCap, setDailyCap] = useState(UNLIMITED);
  const isUnlimited = dailyCap === UNLIMITED;

  const [tpUrl, setTpUrl]     = useState(DEFAULT_TP_URL);
  const [editing, setEditing] = useState(false);
  const [draft, setDraft]     = useState(tpUrl);
      ...
      <div className="rounded-xl border border-border bg-white shadow-sm overflow-hidden">
        <div className="px-5 py-3.5 border-b border-border flex items-center justify-between">
          ...
          <span className={cn(
            ...
            {isUnlimited ? "∞ Unlimited" : `${dailyCap} / day`}
          </span>
        </div>
        <div className="px-5 py-4">
          <div className="flex items-center gap-4 mb-4">
            <p className="text-[40px] font-bold text-foreground leading-none tracking-tight shrink-0 w-16 text-right">
              {isUnlimited ? "∞" : dailyCap}
            </p>
            <div className="flex-1 space-y-1">
              <input type="range" min={0} max={100} step={5} value={dailyCap}
                onChange={e => setDailyCap(Number(e.target.value))}
                className="w-full accent-emerald-500 cursor-pointer" />
              <div className="flex justify-between text-[10px] text-muted-foreground">
                <span>∞ Unlimited</span><span>100 / day</span>
              </div>
            </div>
          </div>
          <div className="grid grid-cols-4 gap-2 mb-3">
            {INVITE_PRESETS.map(p => (
              <button key={p.cap} onClick={() => setDailyCap(p.cap)}
                className={cn("rounded-lg border px-3 py-2 text-center transition-colors",
                  dailyCap === p.cap ? "border-emerald-400 bg-emerald-50" : "border-border bg-white hover:bg-gray-50")}>
                <p className={cn("text-[12px] font-bold", dailyCap === p.cap ? "text-emerald-700" : "text-foreground")}>{p.label}</p>
          ...
          <div className={cn("flex items-center gap-2 text-[11px] rounded-lg px-3 py-2.5",
            ...
            {isUnlimited
              ...
              : <><AlertCircle size={13}/> Capped at {dailyCap}/day · Resets at midnight UTC</>}
          </div>
        </div>
      </div>
